...
def write_output(list_boxes, result_file_path):
    ...
    if isinstance(list_boxes, list):
        for idx, box_data in enumerate(list_boxes):
            ...
            result += line + '\n'
        result = result.rstrip('\n')
    # nếu không phải list hoặc list rỗng => file vẫn được ghi (rỗng)
    with open(result_file_path, 'w', encoding='utf8') as res:
        res.write(result)
...
def main():
    ...
    for idx, img_name in enumerate(list_img_path):
        ...
        output_txt_path = os.path.join(output_txt_dir, img_name_wo_ext + '.txt')
        ...
        img_name_wo_ext = os.path.splitext(os.path.basename(img_name))[0]
        anno_path = os.path.join(anno_dir, img_name_wo_ext + '.txt')
        if not os.path.exists(anno_path):
            print(f"[Warning] Detection annotation not found: {anno_path}")
            continue
