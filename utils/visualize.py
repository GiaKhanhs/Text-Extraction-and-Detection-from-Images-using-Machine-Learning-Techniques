...
def viz_output_of_pick(img_dir, output_txt_dir, output_viz_dir):
    ...
    for n, file in enumerate(list_output_txt):
        img_name = file.replace('.txt', '.jpg')
        img_viz_path = os.path.join(output_viz_dir, img_name)

        # === Skip nếu ảnh đã được visualize rồi ===
        if os.path.exists(img_viz_path):
            print(f"[Skip Viz] {img_name} already exists.")
            continue
