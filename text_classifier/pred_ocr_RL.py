...
from mc_ocr.config import cls_visualize, gpu, cls_ocr_thres
...
def main():
    ...
    for idx, img_name in enumerate(list_img_path):
        ...
        out_txt_path = os.path.join(cls_out_txt_dir, base_name + '.txt')
        if os.path.exists(out_txt_path):
            print(f"Skip {img_name} (already has output)")
            continue
        if idx < 0:
            continue
...
if __name__ == '__main__':
    ...
    for fold in range(3):
        for split in ["train", "val"]:
            ...
            # Gán các path từ fold/split
            img_dir = os.path.join(output_dir, f"rot_out_{split}_img")
            anno_dir = os.path.join(output_dir, f"rot_out_{split}_txt")
            cls_out_txt_dir = os.path.join(output_dir, f"cls_out_{split}_txt")
            cls_out_viz_dir = os.path.join(output_dir, f"cls_out_{split}_viz")

            # Tạo thư mục output nếu chưa có
            os.makedirs(cls_out_txt_dir, exist_ok=True)
            os.makedirs(cls_out_viz_dir, exist_ok=True)

            # Gán global cho main()
            globals()['img_dir'] = img_dir
            globals()['anno_dir'] = anno_dir
            globals()['cls_out_txt_dir'] = cls_out_txt_dir
            globals()['cls_out_viz_dir'] = cls_out_viz_dir
            globals()['img_path'] = ''  # Để dùng toàn bộ folder
            globals()['anno_path'] = ''  # auto resolve in code

            # Gọi hàm main để xử lý fold này
            main()
