...
class OCRDataset(Dataset):
    ...
    def read_data(self, idx):
        ...
        # print(f"Reading image: {img_path} | Index: {idx}")
...
class ClusterRandomSampler(Sampler):
    ...
    def __iter__(self):
        batch_lists = []
        # print("\n=== Debug: Cluster Indices ===")
        for cluster, cluster_indices in self.data_source.cluster_indices.items():
            # print(f"Cluster {cluster}: {len(cluster_indices)} images")
            if self.shuffle:
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        # print("\n=== Debug: Collator Output ===")
        # print(f"Image Tensor Shape: {rs['img'].shape}")
        # print(f"Target Input Shape: {rs['tgt_input'].shape}")
        # print(f"Target Output Shape: {rs['tgt_output'].shape}")
